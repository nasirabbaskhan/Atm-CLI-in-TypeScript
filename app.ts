...
(async () => {
  await showBanner(
    "A T M",
    "ATM allows you to make transection effectivilly and efficientlly ",
    "red",
    "blue"
  );
})();
...
async function fun() {
  ...
  if (answer.accountType && answer.transactionType) {
    ...
    if (balance > userWithdrewAmount) {
      ...
      console.log(
        gradient.rainbow(`Your withdraw amount is :${answer.amount}`)
      );
      console.log(
        gradient.rainbow(`Your Remaining balance is :${remainingBalance}`)
      );
    } else {
